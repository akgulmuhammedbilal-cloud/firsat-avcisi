"""MyDealz kişisel deal-alarm RSS feed okuyucu.

MyDealz, tarayıcı benzeri bir User-Agent olmadan yapılan isteklere HTTP 403
döndürür; bu yüzden istekleri requests + gerçekçi bir User-Agent ile yapıp
gövdeyi feedparser'a veriyoruz.
"""
...
class MyDealzSource(DealSource):
    name = "mydealz"
    ...
    def fetch(self) -> list[RawDeal]:
        ...
        for feed_url in self.feeds:
            try:
                ...
                logger.warning("MyDealz feed çekilemedi (%s): %s", feed_url, exc)
        logger.info("MyDealz: %d ilan çekildi", len(deals))
        return deals
